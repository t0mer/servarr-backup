...
class Lidarr(Server):
    def __init__(self, instance_name):
        ...
        instances = config.get("backups", {}).get("starrs", {}).get("lidarr", [])
        lidarr_config = next((inst for inst in instances if inst['name'] == instance_name), None)
        ...
        url = lidarr_config.get("url")
        api_key = lidarr_config.get("api_key")

        if not url or not api_key:
            raise ValueError("Lidarr URL or API Key is missing in the configuration.")
    ...
    def backup(self):
        ...
        if backup_path:
            # Upload Backup to S3
            storage_key = f"lidarr/{self.instance_name}/{os.path.basename(backup_path)}"
            ...
            if upload_success:
                ...
                # Delete Backup from Lidarr server
    ...
    def create_backup(self):
        ...
        logger.info("Starting backup creation for Lidarr.")
        ...
        if res.status_code == 201:
            ...
            if command_id:
                if self.wait_for_completion(command_id):
                    logger.info("Completed backup for Lidarr successfully.")
                else:
                    logger.error("Failed to complete backup for Lidarr.")
            else:
                logger.error("Failed to retrieve command ID for backup creation.")
        else:
            logger.error(f"Failed to create backup for Lidarr. Status code: {res.status_code}")
            logger.debug(f"Response body: {res.text}")


    def delete_backup(self, backup_name):
        logger.info(f"Deleting backup '{backup_name}' for Lidarr.")
        try:
    ...
    def delete_server_backup(self, backup_id):
        ...
        if res.status_code == 200:
            logger.info(f"Backup {backup_id} deleted successfully from Lidarr.")
        else:
            logger.error(f"Failed to delete the backup {backup_id} from Lidarr. Status code: {res.status_code}")
            logger.debug(res.text)


    def list_backups(self):
        backups = self.storage.list("lidarr")
        backup_list = []
        for backup in backups:
            backup_list.append({
                "Key": backup["Key"],
